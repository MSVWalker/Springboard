...
who signed up. Try not to use the LIMIT clause for your solution. */
    SELECT firstname AS 'First Name', surname AS 'Last Name', joindate AS 'Join Date'
    FROM Members
    WHERE joindate =
        (SELECT MAX(joindate)
         FROM Members);
...
the member name. */
    SELECT
        CONCAT_WS(' ', m.firstname, m.surname) AS 'Member Name',
        CASE
            WHEN COUNT(DISTINCT f.name) > 1 THEN 'Court 1 + Court 2'
            ELSE MAX(f.name)
    ...
    WHERE
        b.facid IN (0, 1)
    GROUP BY
        CONCAT_WS(' ', m.firstname, m.surname);
...
Order by descending cost, and do not use any subqueries. */
    SELECT
        ...
        CONCAT_WS(' ', m.firstname, m.surname) AS 'Member Name',
        CASE
            WHEN m.memid = 0 THEN b.slots * f.guestcost  -- cost for guests
    ...
    HAVING
        Cost > 30  -- only include bookings where cost is more than $30
    ORDER BY
        Cost DESC
